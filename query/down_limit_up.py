...
class LimitUp:
    def __init__(self):
        ...
        self.columns = [u'代码', u'名称', u'当前价格', u'涨幅', u'封单金额/日成交额', u'封单手数/流通股本', u'封单金额', u'第一次跌停时间',
                        u'最后一次跌停时间', u'开盘次数', u'振幅', u'强度']

    # 要爬取的js地址
    def getUrl(self, date):
        return 'http://homeflashdata2.jrj.com.cn/limitStatistic/dtForce/' + date + ".js?_=" + str(
    ...
    def saveData(self, data, date):
        ...
        df = df[(~df['名称'].str.contains('ST|N')) & (df['封单金额/日成交额'] < 1)]
        df = df.sort_values('封单金额/日成交额')
        filename = os.path.join(self.path, "down_limit_up" + ".csv")
        df.to_csv(filename)
        return df.to_csv()
        # try:
